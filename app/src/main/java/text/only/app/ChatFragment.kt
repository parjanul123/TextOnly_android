...
class ChatFragment : Fragment() {
    ...
    private lateinit var btnEmote: ImageButton // New Emoji Button
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        btnEmote = view.findViewById(R.id.btn_emote)
        ...
        lifecycleScope.launch {
            ...
            chatMessages.addAll(messages.map { entity ->
                when (entity.type) {
                    ...
                    "EMOTE" -> EmoteMessage(
                        emoteName = entity.giftName ?: "Emote", // Reuse fields
                        emoteResource = entity.giftResource ?: "ic_coin_shape",
                        isSent = entity.isSent
                    )
    ...
    private fun setupClickListeners() {
        ...
        btnEmote.setOnClickListener { showEmoteSelectionDialog() }
    ...
    // --- New Emote Logic ---
    private fun showEmoteSelectionDialog() {
        val bottomSheet = BottomSheetDialog(requireContext())
        val view = layoutInflater.inflate(R.layout.dialog_select_emote, null)
        bottomSheet.setContentView(view)
        
        val recycler = view.findViewById<RecyclerView>(R.id.recyclerSelectEmote)
        val txtNoEmotesMessage = view.findViewById<TextView>(R.id.txtNoEmotesMessage)
        val btnGoToStore = view.findViewById<Button>(R.id.btnGoToStore)

        recycler.layoutManager = GridLayoutManager(context, 4) // More compact
        
        lifecycleScope.launch {
            val db = AppDatabase.getInstance(requireContext())
            
            // Get OWNED emotes only.
            // Emotes are permanent, type = "EMOTICON"
            // We check inventory
            val inventory = db.storeDao().getInventory().filter { it.itemType == "EMOTICON" }
            
            if (inventory.isEmpty()) {
                recycler.visibility = View.GONE
                txtNoEmotesMessage.visibility = View.VISIBLE
                btnGoToStore.visibility = View.VISIBLE
                ...
                txtNoEmotesMessage.visibility = View.GONE
                btnGoToStore.visibility = View.GONE
                
                recycler.adapter = EmoteSelectAdapter(inventory) { invItem ->
                    sendEmote(invItem)
                    bottomSheet.dismiss()
                }
            }
    ...
    private fun sendEmote(item: InventoryItem) {
        lifecycleScope.launch {
            val db = AppDatabase.getInstance(requireContext())
            val newMessage = MessageEntity(
                contextId = contextId,
                content = "EMOTE: ${item.itemName}",
                isSent = true,
                type = "EMOTE",
                giftName = item.itemName, // Reusing gift fields for simplicity
    ...
    // New Adapter for Emotes
    inner class EmoteSelectAdapter(val items: List<InventoryItem>, val onSelect: (InventoryItem) -> Unit) : RecyclerView.Adapter<EmoteSelectAdapter.Holder>() {
        inner class Holder(v: View) : RecyclerView.ViewHolder(v) {
            val img: ImageView = v.findViewById(R.id.imgEmote)
        }
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): Holder {
            // Need a simple image layout
            // Let\'s create `item_emote_select.xml` or reuse something
            // We can reuse a simple ImageView layout or create one.
            // Assuming we\'ll create `item_emote_select.xml`
            return Holder(layoutInflater.inflate(R.layout.item_emote_select, parent, false))
        }
        override fun onBindViewHolder(holder: Holder, position: Int) {
            val item = items[position]
            holder.img.setImageResource(ResourceMapper.getDrawableId(item.resourceName))
